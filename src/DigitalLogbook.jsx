...
const { user } = useUser();
  ...
  const renderedEntries = useMemo(() => {
    ...
    return filtered.map((entry) => (
      <div key={entry.id} className="relative p-4 rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 flex flex-col gap-3 hover:bg-teal-50/50 dark:hover:bg-teal-900/20 hover:border-teal-300 dark:hover:border-teal-700 hover:shadow-md transition-all duration-200 cursor-pointer"
        onClick={() => setActiveMenuId(activeMenuId === entry.id ? null : entry.id)}
      >
        <div className="flex justify-between items-start gap-3">
          <div className="flex-1 min-w-0">
            <span className="inline-block px-2.5 py-0.5 rounded-full text-xs font-semibold bg-teal-100 dark:bg-teal-900/30 text-teal-800 dark:text-teal-400 mb-2 truncate max-w-full">
              Ref: {entry.REFERENCE_NUMBER}
            </span>
            <h3 className="font-semibold text-slate-900 dark:text-slate-100 break-words line-clamp-2">{entry.PARTICULARS}</h3>
          </div>
            ...
            <div className="py-1">
              {entry.ENCODED_BY === (user?.primaryEmailAddress?.emailAddress || user?.fullName || user?.id) && (
                <button
                  ...
                  <svg className="size-4 text-slate-400 dark:text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-y-2 gap-x-4 text-sm mt-2">
          {entry.ADDRESSE && <div><span className="text-slate-500 dark:text-slate-400 block text-xs uppercase tracking-wider mb-0.5">To</span> <span className="font-medium text-slate-700 dark:text-slate-300">{entry.ADDRESSE}</span></div>}
          {entry.TRANSMITTER && <div><span className="text-slate-500 dark:text-slate-400 block text-xs uppercase tracking-wider mb-0.5">From</span> <span className="font-medium text-slate-700 dark:text-slate-300">{entry.TRANSMITTER}</span></div>}
          {entry.SECTION && <div><span className="text-slate-500 dark:text-slate-400 block text-xs uppercase tracking-wider mb-0.5">Section</span> <span className="font-medium text-slate-700 dark:text-slate-300">{entry.SECTION}</span></div>}
          {entry.MODE_OF_TRANSMITTAL && <div><span className="text-slate-500 dark:text-slate-400 block text-xs uppercase tracking-wider mb-0.5">Mode</span> <span className="font-medium text-slate-700 dark:text-slate-300">{entry.MODE_OF_TRANSMITTAL}</span></div>}
        </div>

        {(entry.REMARKS || entry.ENCODED_BY) && (
          <div className="mt-2 pt-3 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-2 sm:gap-4 text-xs">
            <div className="text-slate-600 dark:text-slate-400 flex-1 break-words w-full">
              {entry.REMARKS && <><span className="font-medium text-slate-500 dark:text-slate-500">Remarks:</span> {entry.REMARKS}</>}
            </div>
            <div className="text-slate-400 dark:text-slate-500 sm:text-right shrink-0 mt-1 sm:mt-0">
    ...
    <div className="h-full flex flex-col overflow-hidden bg-slate-50/50 dark:bg-slate-950/50 text-slate-900 dark:text-slate-100">
      ...
      <div className="p-2 w-full flex-1 flex flex-col gap-8 min-h-0">
        ...
        {showForm && (
          <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-800 p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto relative">
              ...
              {savedReference ? (
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <div className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-6 w-full max-w-sm mb-8">
                    <div className="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Reference Number</div>
                    <div className="text-3xl font-mono font-bold text-teal-700 dark:text-teal-400">{savedReference}</div>
                  </div>
                  <button
                    onClick={handleCloseForm}
                ...
                <div className="flex flex-col gap-4">
                  <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-lg border border-slate-200 dark:border-slate-700">
                    <h3 className="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-3">Receipt Details</h3>
                    <div className="space-y-3">
                      {referenceOverride && (
                        <div>
                          <span className="block text-xs text-slate-400 dark:text-slate-500">Reference Number</span>
                          <span className="font-medium text-slate-900 dark:text-slate-100">{referenceOverride}</span>
                        </div>
                      )}
                      <div>
                        <span className="block text-xs text-slate-400 dark:text-slate-500">Section</span>
                        <span className="font-medium text-slate-900 dark:text-slate-100">{formData.section || <span className="italic text-slate-400 dark:text-slate-500">None</span>}</span>
                      </div>
                      <div>
                        <span className="block text-xs text-slate-400 dark:text-slate-500">Addressee</span>
                        <span className="font-medium text-slate-900 dark:text-slate-100">{formData.addresse || <span className="italic text-slate-400 dark:text-slate-500">None</span>}</span>
                      </div>
                      <div>
                        <span className="block text-xs text-slate-400 dark:text-slate-500">Transmitter</span>
                        <span className="font-medium text-slate-900 dark:text-slate-100">{transmitterName}</span>
                      </div>
                      <div>
                        <span className="block text-xs text-slate-400 dark:text-slate-500">Mode of Transmittal</span>
                        <span className="font-medium text-slate-900 dark:text-slate-100">{formData.modeOfTransmittal || <span className="italic text-slate-400 dark:text-slate-500">None</span>}</span>
                      </div>
                      <div>
                        <span className="block text-xs text-slate-400 dark:text-slate-500">Particulars</span>
                        <span className="font-medium text-slate-900 dark:text-slate-100 whitespace-pre-wrap">{formData.particulars}</span>
                      </div>
                      <div>
                        <span className="block text-xs text-slate-400 dark:text-slate-500">Remarks</span>
                        <span className="font-medium text-slate-900 dark:text-slate-100 whitespace-pre-wrap">{formData.remarks || <span className="italic text-slate-400 dark:text-slate-500">None</span>}</span>
                      </div>
                    </div>
                  </div>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className={`relative ${isDropdownOpen ? 'z-50' : ''}`} tabIndex={0} onBlur={(e) => {
                        ...
                        <div className="relative">
                          <input
                            ...
                            className="w-full px-3 py-2 pr-10 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500"
                          />
                          <button type="button" onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="absolute inset-y-0 right-0 flex items-center pr-2 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 focus:outline-none">
                        ...
                        {isDropdownOpen && (
                          <div className="absolute z-10 mt-1 w-full bg-white dark:bg-slate-800 rounded-md shadow-lg border border-slate-200 dark:border-slate-700 overflow-hidden">
                            <ul className="max-h-60 overflow-auto py-1 text-base text-slate-700 dark:text-slate-200">
                              {sectionsList.filter(sec => sec.name.toLowerCase().includes((formData.section || "").toLowerCase())).map((sec) => (
                                <li
                                  ...
                                  className="cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-slate-50 dark:hover:bg-slate-700"
                                >
                                  {sec.name}
                                </li>
                              ))}
                              ...
                              {!sectionsList.some(sec => sec.name.toLowerCase() === (formData.section || "").trim().toLowerCase()) && (formData.section || "").trim() !== "" && (
                                <li
                                  ...
                                  className="cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-slate-50 dark:hover:bg-slate-700 text-teal-600 dark:text-teal-400 font-semibold border-t border-slate-100 dark:border-slate-700 mt-1"
                                >
                              ...
                              {(formData.section || "").trim() === "" && (
                                <li
                                  ...
                                  className="select-none relative py-2 pl-3 pr-9 text-slate-500 dark:text-slate-400 italic text-sm border-t border-slate-100 dark:border-slate-700 mt-1 cursor-default"
                                >
                    ...
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Addressee</label>
                      <div className={`relative ${isAddresseeDropdownOpen ? 'z-50' : ''}`} tabIndex={0} onBlur={(e) => {
                        ...
                        <div className="relative">
                          <input
                            ...
                            className="w-full px-3 py-2 pr-10 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500"
                          />
                          <button type="button" onClick={() => setIsAddresseeDropdownOpen(!isAddresseeDropdownOpen)} className="absolute inset-y-0 right-0 flex items-center pr-2 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 focus:outline-none">
                            ...
                            <ul className="max-h-60 overflow-auto py-1 text-base text-slate-700 dark:text-slate-200">
                              {recentAddressees.filter(add => add.toLowerCase().includes((formData.addresse || "").toLowerCase())).map((add, idx) => (
                                <li
                                  ...
                                  className="cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center justify-between"
                                >
                                  <div className="flex items-center gap-2 text-slate-700 dark:text-slate-200">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4 text-slate-400 dark:text-slate-500">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Mode of Transmittal</label>
                    <div className={`relative ${isTransmittalDropdownOpen ? 'z-50' : ''}`} tabIndex={0} onBlur={(e) => {
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="w-full px-3 py-2 pr-10 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500"
                        />
                        <button type="button" onClick={() => setIsTransmittalDropdownOpen(!isTransmittalDropdownOpen)} className="absolute inset-y-0 right-0 flex items-center pr-2 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 focus:outline-none">
                  ...
                  <div>
                    ...
                    <textarea required name="particulars" value={formData.particulars} onChange={handleInputChange} rows={3}
                      className="w-full px-3 py-2 border border-slate-300 dark:border-slate-700 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 resize-none" />
                  </div>
        ...
        <div className="flex-1 flex flex-col min-h-0">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 flex-1 flex flex-col min-h-0">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-4 w-full sm:w-auto">
                <div className="relative w-full sm:w-auto">
                  ...
                  <input
                    ...
                    className="pl-9 pr-4 py-2 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 w-full sm:w-64"
                  />
                </div>
                ...
                <div className={`relative ${isDisplayYearDropdownOpen ? 'z-50' : ''}`} tabIndex={0} onBlur={(e) => {
                  ...
                  <div className="relative cursor-pointer" onClick={() => setIsDisplayYearDropdownOpen(!isDisplayYearDropdownOpen)}>
                    <input
                      ...
                      className="w-full sm:w-32 px-3 py-2 pr-8 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 cursor-pointer"
                    />
                    <button type="button" className="absolute inset-y-0 right-0 flex items-center pr-2 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 focus:outline-none pointer-events-none">
                  ...
                  {isDisplayYearDropdownOpen && (
                    <div className="absolute z-10 mt-1 w-full bg-white dark:bg-slate-800 rounded-md shadow-lg border border-slate-200 dark:border-slate-700 overflow-hidden">
                      <ul className="max-h-60 overflow-auto py-1 text-sm text-slate-700 dark:text-slate-200">
                        <li
            ...
            <div className="flex-1 overflow-y-auto pr-2 space-y-4">
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center h-64 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-lg text-slate-400 dark:text-slate-500">
      ...
      {showExportModal && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex justify-center items-center z-[100] p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-200 dark:border-slate-800 w-full max-w-md p-6 relative">
            <button onClick={() => setShowExportModal(false)} className="absolute top-4 right-4 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
            ...
            <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-4">Export Logbook</h2>
            <div className="flex flex-col gap-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Filter Type</label>
                <div className={`relative ${isExportFilterDropdownOpen ? 'z-50' : ''}`} tabIndex={0} onBlur={(e) => {
                  ...
                  <div className="relative cursor-pointer" onClick={() => setIsExportFilterDropdownOpen(!isExportFilterDropdownOpen)}>
                    <input
                      ...
                      className="w-full px-3 py-2 pr-10 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 cursor-pointer"
                    />
                    <button type="button" className="absolute inset-y-0 right-0 flex items-center pr-2 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 focus:outline-none pointer-events-none">
